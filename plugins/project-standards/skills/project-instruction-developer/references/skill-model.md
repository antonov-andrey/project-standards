...
- Every non-list model run MUST bind model sessions to the exact provider worktree sources. Pass every local provider root through `--plugin-marketplace` and every required exact `NAME@MARKETPLACE` selector through `--plugin`. The runner carries each validated physical provider source root into case evaluation. Before the judge runs, every qualified activated skill MUST resolve to the canonical physical `skills/<skill_name>/SKILL.md` under its exact bound provider source. One unqualified activated skill MAY resolve only to exactly one case-declared qualified provider skill or one physical `.agents/skills/<skill_name>/SKILL.md` owned by the evaluated project. Missing, ambient-only, mismatched, symbolic, and colliding activation identities MUST fail before judging. Required providers MUST include all providers from each selected case's `expected_skill_list` and `forbidden_skill_list`, plus every resolved activated provider. Each selector MUST use closed single-segment identities and name a marketplace and plugin declared by one of those exact local manifests, and the selected plugin source MUST be one physical local directory contained by that marketplace worktree. Server bootstrap installs those exact revisions in the standard Codex home before evaluation. The runner validates a strict SemVer cache identity, physical strict containment below the physical resolved standard cache root, and byte-for-byte equality between each declared source and its installed cache. It rejects missing, ambiguous, ambient-version, remote, escaping, symbolic, non-physical, or different source bindings and MUST NOT install plugins or substitute Codex process state. The process-state boundary is owned by `project-standards:project-foundation`, reference `Codex Process State`.
