...
- External Dockerfile frontends, SBOM or provenance generators, and other builder helper images are build dependencies governed by the same identity contract. Reproducible builds MUST resolve and retain their exact digest before use; an implicit moving default or a tag-only parser, scanner, or helper reference is forbidden.
