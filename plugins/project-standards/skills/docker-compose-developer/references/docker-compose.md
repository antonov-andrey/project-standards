...
- One build or release graph defines exactly one canonical image identity for each runtime or tool family, including build bases and directly deployed third-party images, and every compatible consumer reuses it. Consumer-specific aliases and variants that differ only by distribution, mirror, or flavor are forbidden.
