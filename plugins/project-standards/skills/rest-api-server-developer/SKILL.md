...
Classify each route as Product API or infrastructure API before selecting its owner, access, authentication, OpenAPI metadata, audit behavior, or dependencies. The shared `ProductApiRouter` registration mechanism does not collapse those two classifications. Declare the resulting access before domain execution, keep documentation synchronized with actual routes, preserve canonical field identities, and keep authentication, secret, session, and audit boundaries explicit.
