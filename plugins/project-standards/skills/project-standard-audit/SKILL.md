...
Run `project-standardize --workspace-root <workspace-root> --check` and `project-standard-check --project-root <repository-root> --scope all` as mechanical evidence, then inspect each reported repository semantically. Verify every selected standard is applicable, every applicable standard is selected or explicitly excepted, every provider is available, and project-local rules remain genuine overlays.
