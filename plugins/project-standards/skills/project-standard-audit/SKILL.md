...
description: Run an explicitly requested complete semantic standards-conformance audit of one governed repository or explicit workspace.
---

# Project Standard Audit

Read `references/workspace-inventory.md` before inventorying an explicit workspace. Read `references/project-standard-audit.md` completely before the semantic phase.
...
A clean result is forbidden while any owner, rule family, requirement, path binding, applicability decision, or approved source-to-target ledger entry lacks semantic coverage. When implementation is authorized, fix all findings, rerun applicable mechanical verification, then restart the complete semantic audit from owner discovery. Repeat until a fresh full audit produces no findings.
