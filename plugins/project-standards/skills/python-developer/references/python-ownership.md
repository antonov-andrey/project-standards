...
- When one cohesive subsystem needs multiple modules, the change MUST first create or reuse the smallest package that owns their shared domain or operational context. The subsystem context belongs in the package path; repeating it as a common filename prefix across a broader `lib/`, `tool/lib/`, or project-root directory is forbidden when those modules form one owner family.
- Splitting one overloaded module into a flat fan-out of prefixed sibling files does not satisfy the ownership contract. The target package MUST group real child responsibility families into named subpackages when those families have their own state, lifecycle, or boundary vocabulary, while keeping genuinely shared package primitives at the nearest common package level.
...
- That semantic verification MUST also inspect the target directory and package tree. It MUST reject a refactor when the new files merely exchange one God module for a flat prefixed file collection, a generic bucket, or compatibility facades instead of establishing explicit package ownership.
