...
- A singular domain, product, platform, protocol, or external-boundary token that ends with `s` MUST NOT be renamed only to satisfy a mechanical plural-token checker; such tokens MUST be represented as explicit checker exceptions when the checker cannot infer the singular meaning.
