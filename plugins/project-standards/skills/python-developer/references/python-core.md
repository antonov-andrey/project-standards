...
- When the governed project's declared runtime dependency graph contains one canonical `BaseModelStrict` owner, detached non-ORM contract objects that carry stable field-like data across one boundary or stable handoff, and non-ORM classes that expose stable field-like stored data through their public API, including direct attributes, `property`, getter/setter methods, or `dump`/`snapshot`/`export`-style accessors, MUST inherit from that canonical `BaseModelStrict`.
- A project or self-contained execution surface without a canonical `BaseModelStrict` runtime owner MUST NOT copy, vendor, or independently reimplement `BaseModelStrict`, and MUST NOT add Pydantic only to imitate another project's model foundation. It MUST use the strict idiomatic model of its actual owned runtime dependency boundary: strict Pydantic models for untrusted/configuration/serialized boundaries when Pydantic is already one real runtime dependency, and frozen slotted dataclasses for trusted immutable internal handoffs or dependency-free Python boundaries. A dataclass that receives external payload data MUST be constructed only after one closed shape-and-type parser has produced its final typed field values.
- The no-`BaseModelStrict` case does not permit raw external payloads, mutable field bags, unknown fields, coercion, late normalization, or duplicated validation. One boundary owner MUST reject the complete external shape and construct final typed values before the object crosses its first internal handoff.
...
- A canonical `BaseModelStrict` MUST use only standard `Pydantic BaseModel` runtime semantics with `strict=True`, `validate_assignment=True`, `validate_default=True`, and `extra="forbid"`.
