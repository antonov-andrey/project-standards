...
- The governed project MUST declare its current table lifecycle semantics as a project-local overlay when they are not already owned by a narrower stable domain contract.
- A provider MUST NOT replace one project's mutable-state, snapshot, append-only, retention, precision, or deletion semantics with another project's lifecycle only because their column names overlap.
- Every lifecycle timestamp remains subject to `project-foundation/references/temporal-data.md` and preserves the precision required by its project-local contract.
