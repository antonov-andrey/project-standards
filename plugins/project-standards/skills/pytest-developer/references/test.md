...
- The shared plugin MUST discover the root `test`, every tracked project-local `Skill/test`, and direct-submodule owner-local `test` roots, preserve standalone Skill and Submodule execution and explicit pytest selection, exclude tests from the installed provider distribution, ignored worktrees, virtual environments, caches, build outputs, and unrelated nested repositories, and report deterministic discovery.
