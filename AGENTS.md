...
- Validate the provider behavior corpus deterministically with `plugins/project-standards/skills/project-instruction-developer/scripts/skill_behavior_eval.py --corpus skill_behavior_eval/corpus-v1.json --list`. Before a model phase, server bootstrap installs the exact required provider revisions in the current operating-system user's standard Codex home. Pass each matching provider worktree as `--plugin-marketplace <root>` and each required plugin as `--plugin <name>@<marketplace>` so the runner can prove that the installed cache equals those exact local sources; the runner does not install plugins or substitute Codex process state.
